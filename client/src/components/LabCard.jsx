    ...
    <Link to={`/labs/${lab._id}`} className="lab-card">
      ...
      <div className="lab-card-skills">
        {lab.skills_needed.map((skill) => (
          <span
            key={skill}
            className={`skill-tag ${showMatch && userSkills.some((us) => us.toLowerCase() === skill.toLowerCase()) ? 'skill-match' : ''}`}
          >
            {skill}
          </span>
        ))}
      ...
      <div className="lab-card-footer">
        <div className="lab-card-meta">
          <span className={`funding-badge funding-${lab.funding_status}`}>
...
LabCard.propTypes = {
  lab: PropTypes.shape({
    ...
    skills_needed: PropTypes.arrayOf(PropTypes.string).isRequired,
